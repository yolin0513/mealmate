...
> 狀態：**已被 `docs/SPEC_測試範圍_修訂一.md` 大幅取代（2026-09-19）**——十三版回放沒過驗收，統籌者裁決**不做挑選器**。本檔的 R1–R5、§6 的「新增」、§7、§8 的回放驗收作廢；R6–R9 與 §6 的文件修改照修訂一調整後仍有效。**先讀修訂一。** 本檔留著是為了保存勘查事實與當初的設計理由。**依修訂一已執行（2026-09-19）。**
> 規劃：Fable 5.1 統籌 Session，2026-09-19。方向：Yolin 2026-09-19 裁示、2026-09-19 點名 MealMate 要做。細節的取捨見 §9，Yolin 可以否決其中任何一項。事實依據：Opus 勘查員 2026-09-19 的唯讀勘查（HEAD `58c934c`），以及 TripQuest 2026-09-19 做同一件事的實戰經驗（含它踩過的坑與兩次修訂）。

## 0. 前置條件已滿足（2026-09-19 更新）
