// 一次改完三個地方的版本號（npm run bump -- mealmate-v0.1.1）。
//
// 版本號必須同時出現在 js/version.js、sw.js、index.html 的 ?v= 參數裡。
// 漏改一個，瀏覽器就可能把新舊檔案湊在一起（StockDiary 踩過：按按鈕跳回首頁）。
// shelltest 會斷言三者一致。
...
if (!/^mealmate-v\d+\.\d+\.\d+$/.test(String(next))) {
  console.error('用法：npm run bump -- mealmate-v0.1.1');
  process.exit(1);
}
...
const files = {
  ...
  'index.html': (s) => s.replace(/\?v=mealmate-v\d+\.\d+\.\d+/g, `?v=${next}`),
};
