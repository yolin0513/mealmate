...
const MUTATIONS = [
  ...
  },
  {
    ...
    file: "css/style.css",
    find: ".modal-actions-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 10px; }",
  ...
  {
    name: "過去的日子不再預設收起來（回到改版前）",
    why: "使用者：「今天已經週四，就不要再攤開週一到週三」。",
    file: "js/views/week.js",
    find: "  const dayIsOpen = (day, date) => (folds[day] ? folds[day] === 'open' : !isPastDay(date));",
    replace: "  const dayIsOpen = (day, date) => (folds[day] ? folds[day] === 'open' : true);",
    test: "weekviewtest",
  },
  {
    name: "手動展開過去的日子不被記住",
    why: "點開昨天的菜、一重畫又收回去，使用者會覺得 App 不聽話。",
    file: "js/prefs.js",
    find: "  cur[day] = on ? 'closed' : 'open';",
    replace: "  if (on) cur[day] = 'closed'; else delete cur[day];",
    test: "weekviewtest",
  },
  {
