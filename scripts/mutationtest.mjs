...
const MUTATIONS = [
  ...
  },
  {
    ...
    file: "js/views/recipeedit.js",
    find: "      return hit ? `→ 依名稱對到 ${displayNameOf(hit, idx)}（${hit.cat}）` : `→ 資料庫裡查不到「${typed}」：可以照樣存，這個食材的營養會寫「未估算」`;",
    replace: "      return hit ? `→ 依名稱對到 ${displayNameOf(hit, idx)}（${hit.cat}）` : '尚未選食材';",
    test: "recipeviewtest",
  },
  {
    name: "購物清單漏掉查不到的食材",
  ...
  },
  {
    ...
    file: "js/views/recipeedit.js",
    find: "      return hit ? `→ 依名稱對到 ${displayNameOf(hit, idx)}（${hit.cat}）` : `→ 資料庫裡查不到「${typed}」：可以照樣存，這個食材的營養會寫「未估算」`;",
    replace: "      return hit ? '尚未選食材' : `→ 資料庫裡查不到「${typed}」：可以照樣存，這個食材的營養會寫「未估算」`;",
    test: "recipeviewtest",
  },
  ...
  // ---- 2026-09-18 九項調整第三批：食材簡化、名稱跟著換、直覺單位（第 6 項） ----
  {
    ...
    file: 'js/views/recipeedit.js',
    find: '      const hits = q && idx ? searchFoods(q, idx, 8, { collapse: true }) : [];',
    replace: '      const hits = q && idx ? searchFoods(q, idx, 8) : [];',
    test: 'recipeviewtest',
  },
  {
    name: '收合時沒有跳過細分',
  ...
  {
    name: '改選別的食材時名稱不跟著換（回到改版前）',
    why: 'Yolin 回報的 bug：找食材輸入完、再改其他食材，下方顯示的名稱不會跟著更新。',
    file: 'js/views/recipeedit.js',
    find: '          if (!ing.label.trim() || ing.label === ing.autoLabel) { ing.label = name; ing.autoLabel = name; labelInput.value = name; }',
    replace: '          if (!ing.label.trim()) { ing.label = name; ing.autoLabel = name; labelInput.value = name; }',
    test: 'recipeviewtest',
  },
  {
    name: '改選食材時蓋掉使用者自己改的名稱',
    why: '使用者寫好的「高麗菜絲」被換回「甘藍」。',
    file: 'js/views/recipeedit.js',
    find: '          if (!ing.label.trim() || ing.label === ing.autoLabel) { ing.label = name; ing.autoLabel = name; labelInput.value = name; }',
    replace: '          if (true) { ing.label = name; ing.autoLabel = name; labelInput.value = name; }',
    test: 'recipeviewtest',
  },
  {
